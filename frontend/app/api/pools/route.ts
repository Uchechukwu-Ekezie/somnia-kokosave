...
export async function POST(req: NextRequest) {
  try {
    // Supabase is optional - if not configured, just return success
    // Pool data is stored on-chain and via Somnia Data Streams
    if (!isSupabaseConfigured) {
      const body = await req.json()
      return NextResponse.json(
        { 
          success: true,
          message: 'Pool created on-chain. Supabase not configured - using on-chain storage only.',
          pool: {
...
export async function GET(req: NextRequest) {
  // Check if Supabase is configured FIRST - before any try/catch
  if (!isSupabaseConfigured) {
    return NextResponse.json([])
  }
  ...
  try {
    ...
    if (errorMsg.includes('fetch failed') || errorMsg.includes('ECONNREFUSED') || errorMsg.includes('ENOTFOUND') || errorMsg.includes('Unable to connect to Supabase')) {
      // Supabase connection error - but it's optional, so just return empty array
      console.debug('Supabase not available (optional):', errorMsg)
      return NextResponse.json([])
    }
...
export async function PATCH(req: NextRequest) {
  ...
  if (!isSupabaseConfigured) {
    return NextResponse.json(
      { success: true, message: 'Supabase not configured - updates are on-chain only' },
      { status: 200 }
    )
  }
