...
export function RotationalForm() {
  ...
  const { create, isLoading, isSuccess, hash, poolAddress } = useCreateRotational(
    ...
    formData.frequency,
    50,  // 0.5% treasury fee (50 basis points)
    50   // 0.5% relayer fee (50 basis points)
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validMembers.length < 3) {
      setError("Need at least 3 valid members (0x...) for a rotational pool")
      return
    }
  ...
  useEffect(() => {
    // Pool is saved on-chain - navigate directly using contract address
    if (isSuccess && hash && poolAddress && !hasAttemptedSave.current) {
      hasAttemptedSave.current = true
      // Navigate to pool page using contract address
      setTimeout(() => {
        router.push(`/dashboard/group/${poolAddress}`)
      }, 1000)
    }
  }, [isSuccess, hash, poolAddress, router])
