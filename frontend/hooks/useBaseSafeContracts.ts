...
export function useApproveToken(spender: string, amount: string) {
  ...
  // Use max uint256 for unlimited approval (user only needs to approve once)
  ...
  useEffect(() => {
    if (error) {
      console.error('🔴 TOKEN APPROVAL - Write Contract Error:', {
  ...
  useEffect(() => {
    if (receiptError) {
      console.error('🔴 TOKEN APPROVAL - Receipt Error:', {
        tokenAddress: TOKEN_ADDRESS,
        spender,
        txHash: data,
        error: receiptError,
  ...
  }, [receiptError, spender, data])

  const approve = () => {
    console.log('🔵 TOKEN APPROVAL - Initiating:', {
      tokenAddress: TOKEN_ADDRESS,
      spender,
      amount: 'MAX_UINT256 (unlimited)',
      currentChainId: chain?.id,
      expectedChainId: SOMNIA_CHAIN_ID,
      chainName: chain?.name,
      isCorrectNetwork: chain?.id === SOMNIA_CHAIN_ID
    })
    
    if (chain?.id !== SOMNIA_CHAIN_ID) {
      console.error('🔴 WRONG NETWORK! Expected Somnia (50312), got:', chain?.id, chain?.name)
      alert(`Wrong network! Please switch to Somnia Testnet (Chain ID: ${SOMNIA_CHAIN_ID}). Currently on: ${chain?.name || 'Unknown'} (${chain?.id})`)
      return
    }
...
export function useRotationalDeposit(poolAddress: string) {
  ...
  useEffect(() => {
    if (error) {
      console.error('🔴 ROTATIONAL DEPOSIT - Write Contract Error:', {
        poolAddress,
        error,
        errorMessage: error.message,
  ...
  useEffect(() => {
    if (receiptError) {
      console.error('🔴 ROTATIONAL DEPOSIT - Receipt Error:', {
        poolAddress,
        txHash: data,
        error: receiptError,
        errorMessage: receiptError.message,
  ...
  }, [receiptError, poolAddress, data])

  const deposit = () => {
    console.log('🔵 ROTATIONAL DEPOSIT - Initiating:', {
      poolAddress,
      tokenAddress: TOKEN_ADDRESS,
    ...
    try {
      ...
      console.error('🔴 ROTATIONAL DEPOSIT - Exception:', err)
    }
  }
